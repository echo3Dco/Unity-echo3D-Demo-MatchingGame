...
public class RemoteTransformations : MonoBehaviour
{
    ...
    private bool receivedRemotePosition = false;
    private bool receivedRemoteScale = false;
    private bool receivedRemoteRotation = false;
    ...
    public void Update()
    {
        ...
        if (!applyPositionOffset && receivedRemotePosition)
        {
            receivedRemotePosition = false;
            this.gameObject.transform.localPosition = Vector3.zero;
        }
        else if (applyPositionOffset)
        {
        ...
        if (applyRotation || offset != 0)
        {
            ...
            if (applyRotation)
            {
                receivedRemoteRotation = true;
            }
        }
        else if (!applyRotation && receivedRemoteRotation)
        {
            receivedRemoteRotation = false;
            this.gameObject.transform.rotation = Quaternion.Euler(0, 0 + offset, 0);
        }
